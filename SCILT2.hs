module SCILT2 (SCILabFml(..), LTNode(..), ltIsProvable, startNode) where
...
instance LTSystem LTNode where
    ltSuccessors = SCILT2.ltSuccessors
    ltIsAxiomatic = SCILT2.ltIsAxiomatic
    ltIsProven = SCILT2.ltIsAxiomatic
...
ltNormal (Imply f g) True n = [[LFml f False n], [LFml g True (n+1)]]
